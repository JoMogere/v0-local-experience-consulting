  ...
  return (
    <section className="py-16 md:py-24 bg-navy-deep" id="portfolio">
      <div className="max-w-[1200px] mx-auto px-4 md:px-10">
        <h2 className="text-3xl md:text-[42px] font-bold mb-8 md:mb-10 tracking-[-0.02em] text-[#F9F6F0]">Portfolio</h2>
        <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-6 md:gap-8 mt-8 md:mt-15">
          <div className="relative rounded-sm overflow-hidden h-[280px] md:h-[400px] lg:h-[500px] group order-1 lg:order-none">
            <Image
            ...
            />
            <div className="absolute bottom-0 left-0 right-0 bg-[#0D0D0D]/90 flex flex-col justify-end p-5 md:p-10">
              <h3 className="text-xl md:text-[32px] font-bold text-[#F9F6F0]">
                {properties[activeIndex].name}
              </h3>
              <p className="text-text-gray text-sm md:text-base">{properties[activeIndex].location}</p>
            </div>
          </div>
          <div className="flex flex-row lg:flex-col gap-2 md:gap-5 overflow-x-auto lg:overflow-x-visible pb-2 lg:pb-0 order-2 lg:order-none">
            {properties.map((property, index) => (
              <button
                ...
                className={`bg-navy-alt border p-3 md:p-8 rounded-sm text-left transition-colors duration-[400ms] ease-[cubic-bezier(0.16,1,0.3,1)] cursor-pointer flex-shrink-0 lg:flex-shrink min-w-[140px] lg:min-w-0 ${
                  index === activeIndex
                    ? "border-gold-flat"
                    : "border-border-flat hover:border-gold-flat/40"
                }`}
              >
                <span className="block font-bold text-sm md:text-lg text-[#F9F6F0]">{property.name}</span>
              </button>
            ))}
          </div>
        </div>
